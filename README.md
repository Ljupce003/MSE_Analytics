...
The frontend interacts with the Spring Boot backend through the following endpoints:

| Endpoint                 | Method | Description                                                                    |
|--------------------------|--------|--------------------------------------------------------------------------------|
| `/api/issuers`           | `GET`  | Returns a list of all stock tickers for the dropdowns.                         |
| `/api/chart/{code}`      | `GET`  | Fetches historical price data and technical indicators for a specific stock.   |
| `/api/prediction/{code}` | `GET`  | Retrieves the 7-day forecast generated by the LSTM model.                      |
| `/api/progress`          | `GET`  | The polling endpoint used by the Monitoring page to get real-time script logs. |
